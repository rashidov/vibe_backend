import {
  ChannelRoomRelation,
  ChannelRoomRelationRemovePayload,
} from '../types/channel'
...
class ChannelRoomsEntity {
  ...
  addChannel(newChannel: ChannelRoomRelation) {
    if (this.channels.has(newChannel.channel_id))
  ...
  addRooms(channel: ChannelRoomRelation) {
    if (!this.channels.has(channel.channel_id))
      return this.addChannel(channel)!.rooms
    const rooms = [
      ...this.channels.get(channel.channel_id)!.rooms,
      ...channel.rooms,
    ]
  ...
  removeRoom(payload: ChannelRoomRelationRemovePayload) {
    ...
    const rooms = this.channels
      .get(payload.channel_id)!
      .rooms.filter((roomId) => roomId !== payload.room_id)
    this.channels.set(payload.channel_id, {
